<html>
	<body >
		<table align="center" border="1px" height="90%" width="90%">
			<tr height="50px">
				<td colspan="3">
					<a href="PublicHome.php"><img valign="top" src="picture/HB2.jpg" height="50px" width="150px"></a>
					&nbsp;&nbsp;<font size="10" color="green">HUMMING</font><font size="5" color="green">bird</font>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<a href="PublicHome.php"><font size="5" color="red">Home</font></a> |
					<a href="Login.php"><font size="5" color="red">Login</font></a>     |
					<a href="Registration.php"><font size="5" color="red">Registration</font></a> 
				</td>
			</tr>
			<tr>
				<td colspan="1" valign="top" width="45%" align="center">
				<br/><br/>
					
					
					
					<img src='' height="300px" width="200px">
					<br/><br/>
					
					
					<input type="submit" value="<<<previous"/>
					<a href=""><img src='' height="80px" width="50px"></a>
					<a href=""><img src='' height="80px" width="50px"></a>
					<a href=""><img src='' height="80px" width="50px"></a>
					<a href=""><img src='' height="80px" width="50px"></a>
					<a href=""><img src='' height="80px" width="50px"></a>
					<input type="submit" value="next>>>"/>	
					
				</td>
				<td valign="top">
				<br/><br/><br/>
					<font size="15" color="black"><b>xmobile</b></font><br/>
					<font size="5" color="black">starting price:</font><br/>
					<font size="8" color="black"><b>TK: 0000000000</b></font><br/>
					<font size="5" color="black">cureent price :</font><br/> 
					<font size="10" color="red"><b>TK: #########</b></font><br/> 
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<a href="phonedetails.php"><input type="submit" value="    Bit...   "></a><br/><br/><br/>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<script>
							function countDown(secs,elem) {
							var element = document.getElementById(elem);
							element.innerHTML = "Please wait for "+secs+" seconds for bitting time";
							if(secs < 1) 
							{
								clearTimeout(timer);
								
								window.location.href="congratulation.php";
							}
							secs--;
							var timer = setTimeout('countDown('+secs+',"'+elem+'")',1000);
							}
					</script>
					<div id="status"></div>
			<script>countDown(5,"status");</script>

			
			<tr height="50px" align="center">
			<td colspan="3">Copyright&copy; 2017</td>
			</tr>
		</table>